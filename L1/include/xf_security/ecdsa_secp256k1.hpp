...
#else
    // Elliptic-curve definition parameter for y^2 = x^3 + ax + b in GF(p)
    ap_uint<256> a = ap_uint<256>("0x0");
    /// Elliptic-curve definition parameter for y^2 = x^3 + ax + b in GF(p)
    ap_uint<256> b = ap_uint<256>("0x7");
    /// Elliptic-curve definition parameter for y^2 = x^3 + ax + b in GF(p)
    ap_uint<256> p = ap_uint<256>("0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFC2F");

    /// X coordinate of generation point of curve secp256k1.
    ap_uint<256> Gx = ap_uint<256>("0x79BE667EF9DCBBAC55A06295CE870B07029BFCDB2DCE28D959F2815B16F81798");
    /// Y coordinate of generation point of curve secp256k1.
    ap_uint<256> Gy = ap_uint<256>("0x483ADA7726A3C4655DA4FBFC0E1108A8FD17B448A68554199C47D08FFB10D4B8");
    /// Order of curve secp256k1.
    ap_uint<256> n = ap_uint<256>("0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141");
...
#if 0
    ap_uint<256> productMod_p1(ap_uint<256> a, ap_uint<256> b) {
...
#pragma HLS allocation function instances = internal::addMod limit = 2
#endif
        ap_uint<128> aH = a.range(255, 128);
        ap_uint<128> aL = a.range(127, 0);
        ap_uint<128> bH = b.range(255, 128);
...
#ifdef SET_FUNC_LIMIT
#pragma HLS allocation function instances = productMod_p4 limit = 1
#endif
	    return productMod_p4(a, b);
    }

#ifndef MOD_N_ALT
    ap_uint<256> productMod_n(ap_uint<256> a, ap_uint<256> b) {
#ifdef SET_FUNC_LIMIT
#pragma HLS allocation function instances = internal::productMod limit = 1
#endif
// XXX: a * b % n is only called a few times, no need to use specialized version
//#ifndef DONT_USE_INLINE
//  If you remove the following pragma inline, vivado_hls generates seg fault.  JYKIM 21/11/25
#pragma HLS inline
//#endif
...
#ifdef SET_FUNC_LIMIT
#pragma HLS allocation function instances = productMod_p limit = 2
#pragma HLS allocation function instances = internal::modularInv<256> limit = 1
#endif
#ifdef DONT_USE_INLINE
#pragma HLS inline off
#else
...
#pragma HLS allocation function instances = internal::addMod<256> limit = 2
#pragma HLS allocation function instances = internal::subMod<256> limit = 2
#endif
#ifdef DONT_USE_INLINE
#pragma HLS inline off
#else
...
#ifdef SET_FUNC_LIMIT
#pragma HLS allocation function instances = productMod_p limit = 1
#pragma HLS allocation function instances = doubleJacobian limit = 2
#pragma HLS allocation function instances = internal::subMod<256> limit = 2
#endif
#ifdef DONT_USE_INLINE
#pragma HLS inline off
...
#ifdef SET_FUNC_LIMIT
#pragma HLS allocation function instances = addJacobianAffine limit = 1
#pragma HLS allocation function instances = doubleJacobian limit = 1
#pragma HLS allocation function instances = fromJacobian limit = 1
#pragma HLS allocation function instances = toJacobian limit = 1
#endif
#ifdef DONT_USE_INLINE
#pragma HLS inline off
#else
...
#else
        ap_uint<256> k, ap_uint<256>& Rx, ap_uint<256>& Ry)
#endif
	{
//#pragma HLS allocation instances=mul limit=128 operation
#ifdef SET_FUNC_LIMIT
#pragma HLS allocation function instances = addJacobianAffine limit = 1
#pragma HLS allocation function instances = addJacobian limit = 1
#pragma HLS allocation function instances = fromJacobian limit = 1
#pragma HLS allocation function instances = getNAF limit = 1
#endif
//#pragma HLS inline
#if 1
...
#pragma HLS bind_storage variable = a type = ROM_1P impl = LUTRAM
#pragma HLS bind_storage variable = b type = ROM_1P impl = LUTRAM
#pragma HLS bind_storage variable = p type = ROM_1P impl = LUTRAM
#pragma HLS bind_storage variable = Gx type = ROM_1P impl = LUTRAM
#pragma HLS bind_storage variable = Gy type = ROM_1P impl = LUTRAM
#pragma HLS bind_storage variable = n type = ROM_1P impl = LUTRAM
#endif
...
#ifdef SET_FUNC_LIMIT
#pragma HLS allocation function instances = productMod_n limit = 1
#pragma HLS allocation function instances = dotProductNAFPrecomputeJacobian limit = 1
#pragma HLS allocation function instances = productMod_p limit = 1
#pragma HLS allocation function instances = internal::modularInv<256> limit = 2
#pragma HLS allocation function instances = internal::addMod<256> limit = 2
#endif
        ap_uint<256> x, y;
#if 0
...
#else
        this->dotProductNAFPrecomputeJacobian(k, x, y);
#endif
...
#ifdef SET_FUNC_LIMIT
#pragma HLS allocation function instances = productMod_p limit = 1
#pragma HLS allocation function instances = internal::modularExp<256> limit = 2
#pragma HLS allocation function instances = internal::subMod<256> limit = 2
#endif
    ...
    sqrt_a = xf::security::internal::modularExp<256, 256>(base, p_1_d4, p, rMod);

#ifdef S_DEBUG
std::cout << std::hex << "SQ : " << sqrt_a << "\n";
#endif
    if ( (sqrt_a&1) ^ (recid&1)) {
	    sqrt_a = xf::security::internal::subMod<256>(0, sqrt_a, p);
#ifdef S_DEBUG
std::cout << std::hex << "NSQ: " << sqrt_a << "\n";
     ...
     */
    bool decompress(ap_uint<256> sigr, ap_uint<256>& sqrt_y2, int recid) {
#ifdef SET_FUNC_LIMIT
#pragma HLS allocation function instances = productMod_p limit = 1
#pragma HLS allocation function instances = modularSqrt limit = 2
#pragma HLS allocation function instances = internal::addMod<256> limit = 2
#endif

#ifdef S_DEBUG
        std::cout << std::hex << "decompress recid: " << recid << "\n";
#endif
        if (recid & 2) {
    	    if ( sigr >= p_minus_n)
    		    return false;
        ...
        ap_uint<256> tx1 = productMod_p(sigr, sigr);
#ifdef S_DEBUG
        std::cout << std::hex << "r^2: " << tx1 << "\n";
#endif
        tx1 = productMod_p(tx1, sigr);
#ifdef S_DEBUG
        std::cout << std::hex << "r^3: " << tx1 << "\n";
#endif

        ap_uint<256> tx2 = productMod_p(sigr, a);
        tx2 = xf::security::internal::addMod<256>(tx2, b, p);
...
#define V_RECOVER	1
    ...
    bool recover(ap_uint<256> r, ap_uint<256> s, ap_uint<256> z, ap_uint<256>& Px, ap_uint<256>& Py) {
#ifdef SET_FUNC_LIMIT
#pragma HLS allocation function instances = productMod_n limit = 2
#pragma HLS allocation function instances = dotProductNAFPrecomputeJacobian limit = 1
#pragma HLS allocation function instances = dotProductJacobian limit = 1
#pragma HLS allocation function instances = internal::modularInv<256> limit = 1
#pragma HLS allocation function instances = internal::subMod<256> limit = 1
#endif
                ...
                if ( Px >= 4 )
		    return false;
		if (!decompress(r, rx, Px&0x3))
			return false;
		ap_uint<256> rInv = xf::security::internal::modularInv<256>(r, n);
#ifdef S_DEBUG
		std::cout << std::hex << "RN: " << rInv << "\n";
#endif

		ap_uint<256> tu1 = productMod_n(rInv, z);
#ifdef S_DEBUG
		std::cout << std::hex << "z:  " << z << "\n";
		std::cout << std::hex << "U1: " << tu1 << "\n";
#endif

		ap_uint<256> u1 = xf::security::internal::subMod<256>(0, tu1, n);
#ifdef S_DEBUG
		std::cout << std::hex << "NU1:" << u1 << "\n";
#endif

		ap_uint<256> u2 = productMod_n(rInv, s);
#ifdef S_DEBUG
		std::cout << std::hex << "U2: " << u2 << "\n";
#endif

//		ap_uint<256> rPx, rPy;
		dotProductNAFPrecomputeJacobian(u1, t1x, t1y);  // t1x, t1y = G*u1
		dotProductJacobian(r, rx, u2, t2x, t2y);  // t2x, t2y = [xj]*u2
		add(t1x, t1y, t2x, t2y, Px, Py);
#ifdef S_DEBUG
		std::cout << std::hex << "Px: " << Px << "\n";
		std::cout << std::hex << "Py: " << Py << "\n";

		std::cout << std::hex << "PmN:" << p - n << "\n";
#endif
		return true;
    }
...
#ifdef SET_FUNC_LIMIT
#pragma HLS allocation function instances = productMod_p limit = 2
#pragma HLS allocation function instances = internal::productMod<256> limit = 1
#pragma HLS allocation function instances = dotProductNAFPrecomputeJacobian limit = 1
#pragma HLS allocation function instances = dotProductJacobian limit = 1
#pragma HLS allocation function instances = add limit = 2
#endif
        if (r == 0 || r >= n || s == 0 || s >= n) {
            return false;
        } else {
